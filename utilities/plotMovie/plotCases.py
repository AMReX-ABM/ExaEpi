...
if len(argList) < 5:  # 5 is the minimum mandatory arguments
    ...
    exit
if len(argList) >= 1:
    infCasesDir = argList[0] + "/"
if len(argList) >= 2:
    CensusDir = argList[1] + "/"
if len(argList) >= 3:
    outputMetric = argList[2]
if len(argList) >= 4:
    datanorm = argList[3]
if len(argList) >= 5:
    group = argList[4]
if len(argList) >= 6:
    zoom = argList[5]  # format: "startTime, stopTime, target x, target y, zoomFactor"
if len(argList) >= 7:
    BayAreaDataFormat = argList[6]
...
for file in os.listdir(CensusDir):
    ...
    filename = CensusDir + "/" + file.split(".")[0]
...
for caseFile in sorted_infCasesFiles:
    ...
    if zoom != "NO":
        ...
        if idx >= startTime and idx < (startTime + shiftTime):
            ...
            ax.set_xlim(
                (
                    xlim[0] - (xlim[1] - xlim[0]) * xrate * (idx - startTime),
                    xlim[1] - (xlim[1] - xlim[0]) * xrate * (idx - startTime),
                )
            )
            ax.set_ylim(
                (
                    ylim[0] - (ylim[1] - ylim[0]) * yrate * (idx - startTime),
                    ylim[1] - (ylim[1] - ylim[0]) * yrate * (idx - startTime),
                )
            )
        if idx >= (startTime + shiftTime) and idx < stopTime:
            ...
            ax.set_xlim(
                (
                    xlim[0] + (xlim[1] - xlim[0]) * zoomrate * (idx - startTime - shiftTime) / 2,
                    xlim[1] - (xlim[1] - xlim[0]) * zoomrate * (idx - startTime - shiftTime) / 2,
                )
            )
            ax.set_ylim(
                (
                    ylim[0] + (ylim[1] - ylim[0]) * zoomrate * (idx - startTime - shiftTime) / 2,
                    ylim[1] - (ylim[1] - ylim[0]) * zoomrate * (idx - startTime - shiftTime) / 2,
                )
            )
        if idx >= stopTime:
            ...
            ax.set_xlim(
                (
                    xlim[0] + (xlim[1] - xlim[0]) * zoomrate * (stopTime - shiftTime - startTime) / 2,
                    xlim[1] - (xlim[1] - xlim[0]) * zoomrate * (stopTime - shiftTime - startTime) / 2,
                )
            )
            ax.set_ylim(
                (
                    ylim[0] + (ylim[1] - ylim[0]) * zoomrate * (stopTime - shiftTime - startTime) / 2,
                    ylim[1] - (ylim[1] - ylim[0]) * zoomrate * (stopTime - shiftTime - startTime) / 2,
                )
            )
