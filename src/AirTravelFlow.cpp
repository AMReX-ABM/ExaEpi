...
void AirTravelFlow::readAirTravelFlow (const std::string fname /*!< Filename to read case data from */) {
    ...
    while (nconnections > c++) {
        ...
        if (destPax.find(dest) == destPax.end()) {
            destPax[dest] = pax_per_day;
        } else {
            destPax[dest] += pax_per_day;
        }
    }
...
void AirTravelFlow::computeTravelProbs (const AirTravelDemoData& demo) {
    ...
    for (int i = 0; i < demo.Nunit; i++) {
        int fips = demo.FIPS[i];
        if (originPax.find(FIPS_to_airport[fips]) == originPax.end()) {
            air_travel_prob[i] = 0;
        } else {
            air_travel_prob[i] = (float)originPax[FIPS_to_airport[fips]] / inAirportRangePop[FIPS_to_airport[fips]];
        }
        ...
        if (FIPS_to_airport.find(fips) != FIPS_to_airport.end()) {
            ...
            assigned_airport[i] = -1; // a unit that does not have any airport assigned to it
        }
    }

    {
        ...
        for (airTravelType::iterator it = travel_path_prob.begin(); it != travel_path_prob.end(); it++) {
            ...
            for (std::map<std::string, float>::iterator it1 = it->second.begin(); it1 != it->second.end(); it1++) {
                ...
                // amrex::Print() <<" Original Airport "<< org<< " Dest Airport "<< dest<< " travel path prob threshold "<<
                // travel_path_prob[org][dest]<<"\n";
            }
        }
        ...
        for (int i = 0; i < nairports; i++) {
            ...
            for (int j = 0; j < numDest; j++) {
                ...
                if (j == 0) {
                    dest_airports_prob[curOffset] = travel_path_prob[airport][destAirport];
                } else {
                    dest_airports_prob[curOffset] = dest_airports_prob[curOffset - 1] + travel_path_prob[airport][destAirport];
                }
                // amrex::Print() <<" Original Airport "<< airport<<" Dest Airport "<< destAirport<< " travel path prob threshold
...

    {
        ...
        for (int i = 0; i < nairports; i++) {
            ...
            for (int j = 0; j < nUnits; j++) {
                ...
                if (j == 0) {
                    arrivalUnits_prob[curOffset] = (float)demo.Population[destUnit] / inAirportRangePop[airport];
                } else {
                    arrivalUnits_prob[curOffset] =
                            arrivalUnits_prob[curOffset - 1] + (float)demo.Population[destUnit] / inAirportRangePop[airport];
                }
                // amrex::Print() <<" At Airport "<< airport <<" prob threshold to visit unit "<< destUnit<<" is "<<
                // arrivalUnits_prob[curOffset] <<"\n";
                curOffset++;
            }
